...
SELECT
  ...
  CAST(SYSDATETIME() AS TIME(0)) AS [current_time(0)],
  CAST(SYSDATETIME() AS TIME(2)) AS [current_time(2)],
  CAST(SYSDATETIME() AS TIME(7)) AS [current_time(7)];
...
-- DATEPART
SELECT 
DATEPART(DAY,'20170212') AS [Day],
DATEPART(MONTH,'20170212') AS [Month],
DATEPART(M,'20170212') AS [Month],
DATEPART(MM,'20170212') AS [Month],
DATEPART(YEAR,'20170212') AS [Year],
DATEPART(YY,'20170212') AS [Year],
DATEPART(YYYY,'20170212') AS [Year],
DATEPART(Y,'20170212') AS [Day of Year],
DATEPART(DAYOFYEAR,'20170212') AS [Day of Year],
DATEPART(WEEKDAY,'20170212') AS [Weekday];
...
--using COALESCE (standards-based)
SELECT empid, country, region, city, 
...
ELSE
  PRINT CONCAT('Employee ', CAST(@empid AS VARCHAR(10)), ' was found.');

---
DECLARE @empidOther AS INT = 9;

SELECT empid, firstname, lastname
FROM HR.Employees
WHERE empid = @empidOther;

IF @@ROWCOUNT = 0
  PRINT CONCAT('Employee ', CAST(@empidOther AS VARCHAR(10)), ' was not found.')
ELSE
  PRINT CONCAT('Employee ', CAST(@empidOther AS VARCHAR(10)), ' was found.');
...
-- it fails
SELECT NEWSEQUENTIALID() AS myseqguid1;

--this example works
DROP TABLE myTable;
CREATE TABLE myTable (ColumnA nvarchar(10), SeqGuid uniqueidentifier DEFAULT NEWSEQUENTIALID());  
INSERT INTO myTable (ColumnA) VALUES ( 'aaaaa'), ('bbbbb'), ('ccccc'), ('ddddd'), ('eeeee'), ('fffff'), ('ggggg'), ('hhhhh'), ('iiiii'), ('jjjjj');
...
DECLARE @p1 AS INT = 9, @p2 AS INT = 2;
DECLARE @result AS NUMERIC(12,2);
SET @result = CAST(@p1 AS NUMERIC(12, 2)) / CAST(@p2 AS NUMERIC(12, 2));
SELECT @result;
GO
